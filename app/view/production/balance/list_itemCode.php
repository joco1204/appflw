...
<div class="container theme-showcase" style="background-color:#FFF;">
          ...
          <table style="white-space: nowrap; width: 100%">
            <tr>
              <td style="text-align: left;">
                ...
                <button class="js-cal-option btn btn-default {{: first.toDateInt() <= today.toDateInt() && today.toDateInt() <= last.toDateInt() ? 'active':'' }}" data-date="{{: today.toISOString()}}" data-mode="week">Semana Actual</button>
              </td>
              <td>
...
//date1 = new Date();
date1 = new Date('2018-08-21 00:00');
var data = [],
    date = new Date(),
    d = date.getDate(),
    d1 = d,
    ...
    h, 
    m,
    names = ['PRoDUCCION DIA', 'POSCOSECHA', 'DIA PaR', 'Repetir', 'Prueba1', 'Prueba2', 'Hola', 'Date', 'ID ORDER'],
    slipsum = ["sadasdwdew"];
  ...
  /*for(i = 0; i < 500; i++) {
	...
	if (end != null){
		data.push({ title: "probando"+end, start: new Date(y, m, d, h, m), end: end, allDay: true, text: slipsum[c % slipsum.length ]  });
	}
  ...
  for(i = 0; i < 200; i++) {
	...
	date1r = new Date('2018-07-21 00:00');
	var suma = sumarDias(date1r, +i);
	  var x = Math.floor((Math.random() * 10) + 1);
			for(w = 0; w < x; w++) {
				data.push({ title: "Producto"+i+x+w, start: date1r, end: date1r, allDay: true, text: "texto"+i+x+w });
			}
  }	

  
  data.push({ title: "Product 435", start: date1, end: date1, allDay: true, text: "texto2" });
  data.push({ title: "Product 65", start: date1, end: date1, allDay: true, text: "texto3" });
  data.push({ title: "Product 3343", start: date1, end: date1, allDay: true, text: "texto4" });
